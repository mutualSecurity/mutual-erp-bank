...
class WizardMissingVisits(osv.TransientModel):
    ...
    def createvisits(self):
        ...
        self.env.cr.execute(
            "select visit_time,visit_time_two from mutual_guard_tracking where visit_time is null and visit_time_two is null and bank_code="+"'"+self.bank_code+"'")
        ...
        if(len(missing_branches)>0):
            if(len(missing_visits_created) == 0):
                ...
                raise osv.except_osv('Ghor Ye Dekho......', 'Already missing visits create ho chukey hain....')
        else:
