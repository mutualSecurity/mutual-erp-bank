...
class mutual_projects(osv.osv):
    ...
    _columns = {
        ...
        'reviewer_id': fields.many2one('res.users', 'Forwarded to', select=True, track_visibility='onchange',
                                       domain="[('is_technician','=',False)]"),
        'user_id': fields.many2one('res.users', 'Assigned Tech', select=True, track_visibility='onchange',
        ...
        'bank_code_task': fields.related('partner_id', 'bank_code', type='char', size=12, string='Bank Code',
                                         readonly=True),
        'monitoring_address_task': fields.related('partner_id', 'street', type='char', string='Monitoring address',
                                                  readonly=True),
        ...
        'compute_total_time': fields.char('Total Time', store=True, readonly=True, compute='_compute_total_time',
                                          old='total_time'),
        'priority': fields.selection([('0', 'Normal'), ('1', 'Urgent'), ('2', 'Most Urgent')], 'Priority', store=True,
                                     select=True),
...
class mutual_issues(osv.osv):
    ...
    _columns = {
        ...
        'cs_number_issue': fields.related('partner_id', 'cs_number', type='char', size=12, string='CS Number',
                                          readonly=True),
        'city_issue': fields.related('partner_id', 'city', type='char', size=12, string='City', readonly=True),
        'branch_code_issue': fields.related('partner_id', 'branch_code', type='char', size=12, string='Branch Code',
                                            readonly=True),
        'bank_code_issue': fields.related('partner_id', 'bank_code', type='char', size=12, string='Bank Code',
                                          readonly=True),
        'monitoring_address_issue': fields.related('partner_id', 'street', type='char', string='Bank address',
                                                   readonly=True),
        'remark_by_cms': fields.text('Remarks By CMS', store=True),
        ...
        'user_id_issue': fields.many2one('res.users', 'Forwarded to', required=False, select=1,
                                         track_visibility='onchange', domain="[('is_technician','=',False)]"),
        'user_id': fields.many2one('res.users', 'Assigned Technician', required=False, select=1,
                                   track_visibility='onchange', domain="[('is_technician','=',True)]"),
        'compute_total_time': fields.char('Total Time', store=True, readonly=True, compute='_compute_total_time',
                                          old='total_time'),
        ...
        'name': fields.selection([("Activation & Deactivation Problem	(I)", "Activation & Deactivation Problem (I)"),
                                 ...
                                 'Complaint Title', required=True, read=['__export__.res_groups_52'],
                                 write=['project.group_project_user'],
                                 on_change='type()'),
        ...
        'gsm_postpaid_prepaid': fields.selection([('Prepaid', 'Prepaid'), ('Postpaid', 'Postpaid')], 'Prepaid/Postpaid',
                                                 store=True),
        'ptcl': fields.char('PTCL', store=True),
        'ptcl_dedicated_shared': fields.selection([('Dedicated', 'Dedicated'), ('Shared', 'Shared')],
                                                  'Dedicated/Shared', store=True),
        ...
        'pending': fields.boolean('Pending', store=True, read=['project.group_project_manager'],
                                  write=['project.group_project_manager']),
    ...
    def create(self, vals):
        self.env.cr.execute(
            "SELECT project_issue.name,project_issue.partner_id,project_task_type.name FROM project_issue INNER JOIN project_task_type ON project_issue.stage_id = project_task_type.id WHERE (project_task_type.name != 'Resolved' and project_task_type.name != 'Online Resolved') and project_issue.check = 'Issue' and project_issue.partner_id=" + str(
                vals['partner_id']))
        ...
        if not self.tech and self.stage_id.name == "Assigned to Technician":
            ...
            if self.tech and self.id:
                for line in self.tech_name:
                    self.env.cr.execute(
                        'UPDATE tech_activities_issues SET status=' + "'" + self.stage_id.name + "'" + ' WHERE tech_name = ' + str(
                            self.id))
    ...
    def create_sms(self):
        list_of_sms = []
        rec_dict = {'sms': self.sms, 'contact': self.techContact, 'status': 'sending',
                    'name': self.technician_name.name}
    ...
    _columns = {
        'bas': fields.selection([('GSM Only', 'GSM Only'), ('Bentel Only', 'Bentel Only'),
                                 ('PTCL Only', 'PTCL Only'), ('Bentel and PTCL', 'Bentel and PTCL')
                                    , ('GSM and PTCL', 'GSM and PTCL'), ('Nil', 'Nil')], string='BAS Connected To',
                                store=True),
        ...
        'pending': fields.selection([('Yes', 'Yes'), ('No', 'No'), ('Nil', 'Nil')], string='Work is Pending',
                                    store=True),
        ...
        'monitoring_address_issue': fields.related('tech_name', 'monitoring_address_issue', type='char',
                                                   string='Address'),
        ...
        'status': fields.selection(
            [('Time In/Out', 'Time In/Out'), ('Resolved', 'Resolved'), ('Under Process', 'Under Process'),
             ('Issue at bank end', 'Issue at bank end'),
             ('Additional/Device Replacement', 'Additional/Device Replacement'),
             ('Assigned to Technician', 'Assigned to Technician'), ('Complaints/Tasks', 'Complaints/Tasks'),
             ('Online Resolved', 'Online Resolved'), ('Inventory Check', 'Inventory Check')], 'Complaint Marking',
            store=True, onchange='changestatus()'),
    }
...
class low_messages(osv.osv):
    ...
    _columns = {
        ...
        'employee_name': fields.many2one('hr.employee', 'Technician Name',
                                         domain="[('department_id','=','Technician')]", defaults='',
                                         old='technician_name'),
...
class tech_activities_tasks(osv.osv):
    ...
    _columns = {
        ...
        'technician_name_tasks': fields.many2one('hr.employee', 'Technician Name', required=False, select=1,
        ...
        'complaint_reference': fields.related('tech_name_tasks', 'complaint_reference', type='char',
...
class basicPackage(osv.osv):
    ...
    _columns = {
        'bank': fields.many2one('res.partner', 'Bank', store=True,
                                domain=[('is_company', '=', True), ('is_branch', '=', False)]),
...
class basicPackageItems(osv.osv):
    ...
    _columns = {
        ...
        'issue_product_details': fields.related('req_slip', 'title', type='char', string='Issue Product Details',
                                                readonly=True),
        ...
        'type': fields.selection([('For Technician', 'For Technician'), ('For Customer', 'For Customer'),
                                  ('Handover To Warehouse', 'Handover To Warehouse')], 'Type',
                                 store=True),
...
class couriersheet(osv.osv):
    ...
    _columns = {
        ...
        'state': fields.selection([('draft', 'Draft'), ('confirmed', 'Confirmed')], 'State', store=True,
                                  default='draft', track_visibility='onchange'),
        'ref': fields.char('Ref', store=True, compute='devices_details'),
        'location': fields.related('technician_name', 'work_location', type='char', string='Technician Work Address',
                                   readonly=True)
    }
    ...
    def auto_select(self, context=None):
        if self.complaint_reference:
            self.env.cr.execute(
                'select id from res_partner where id = any(select partner_id from project_issue where id =' + str(
...
class attendance_logs(osv.osv):
    ...
    def cron_tech_attendance(self, cr, uid, context=None):
        ...
        for employee in employees:
            cr.execute(
                "select * from attendance_logs where contact='" + str(employee['work_phone']) + "'" + "and date_='" +
                str(datetime.now()).split(' ')[0] + "'" + "limit 1")
            present_employee = cr.dictfetchall()
            if len(present_employee) > 0:
                res = {
                    'employee_id': employee['id'],
                    'check_in': present_employee[0]['date_'] + " " + present_employee[0]['time_'],
                    'check_in_view': datetime.strptime(
                        str(present_employee[0]['date_'] + " " + present_employee[0]['time_']),
